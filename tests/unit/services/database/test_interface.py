...
class TestDatabaseServiceGetSession:
    ...
    async def test_get_session_raises_when_not_started(self):
        config = DatabaseConfig()
        service = DatabaseService(config=config)
    ...
    async def test_get_session_commits_on_success(self):
        ...
        async for _session in service.get_session():
            pass

        mock_session.commit.assert_awaited_once()

    async def test_get_session_rolls_back_on_error(self):
        """Simulate FastAPI's athrow() behavior on async generator dependency."""
        config = DatabaseConfig()
        service = DatabaseService(config=config)
        ...
        # Use the generator protocol directly (mimics FastAPI Depends behavior)
        gen = service.get_session()
        session = await gen.__anext__()
        assert session is mock_session

        # Throw exception into the generator (what FastAPI does on endpoint error)
