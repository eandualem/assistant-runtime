...
| AG-UI adapter | `ag-ui-protocol>=0.1.10` (the `ag-ui` extra; 0.1.22 locked for development) | `tests/compatibility/test_agui.py`: `RunAgentInput` parsing (`AGUIAdapter.build_run_input`) and event encoding (`AGUIEventStream.transform_stream`) over the runtime's turn pipeline; frontend tool round trip, failed results, duplicates, 422/501 |
...
| Deferred host actions | **Keep composing** `ExternalToolset`, `DeferredToolRequests`, `DeferredToolResults` | Host continuations keep the same assistant message; several host calls from one response are handed over sequentially, with at most one active pending call, and the model resumes after every result is recorded |
