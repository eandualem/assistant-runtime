...
async def create_swarm(
    ...
    return await BackboneWorkgroups().create_swarm(
...
async def list_swarms(repo: str | None = None, status: str | None = None) -> dict[str, Any]:
    ...
    return await BackboneWorkgroups().list_swarms(repo=repo, status=status)


async def get_swarm_detail(swarm_id: str) -> dict[str, Any]:
    """Get full detail for a single swarm."""
    return await BackboneWorkgroups().get_swarm_detail(swarm_id=swarm_id)
...
async def update_worker_status(
    ...
    return await BackboneWorkgroups().update_worker_status(
...
async def broadcast_to_swarm(swarm_id: str, from_entity: str, message: str) -> dict[str, Any]:
    ...
    return await BackboneWorkgroups().broadcast_to_swarm(
        swarm_id=swarm_id, from_entity=from_entity, message=message
    )


async def complete_swarm(swarm_id: str) -> dict[str, Any]:
    """Mark a swarm as completed/cleaned up."""
    return await BackboneWorkgroups().complete_swarm(swarm_id=swarm_id)
...
class BackboneWorkgroups:
    ...
    async def create_swarm(
        ...
        status, data = await self._request("POST", "/api/swarms", json_body=body)
        if status == -1:
    ...
    async def list_swarms(
        ...
        status_code, data = await self._request("GET", "/api/swarms", params=params or None)
        if status_code == -1:
            return {"error": backbone_error(data), "success": False}
        if status_code != 200:
            detail = backbone_detail(data) if isinstance(data, dict) else str(data)
    ...
    async def get_swarm_detail(self, swarm_id: str) -> dict[str, Any]:
        ...
        status, data = await self._request("GET", f"/api/swarms/{normalized_swarm_id}")
        if status == -1:
            return {"error": backbone_error(data), "success": False}
        if status == 404:
    ...
    async def update_worker_status(
        ...
        response_status, data = await self._request(
            "POST",
    ...
    async def complete_swarm(self, swarm_id: str) -> dict[str, Any]:
        ...
        normalized_swarm_id = swarm_id.strip()
        status, data = await self._request("DELETE", f"/api/swarms/{normalized_swarm_id}")
        if status == -1:
            return {"error": backbone_error(data), "success": False}
        if status == 404:
            return {"error": f"Swarm '{normalized_swarm_id}' not found", "success": False}
