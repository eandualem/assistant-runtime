...
async def get_agent_activity(
    ...
    return await BackboneActivity().get_agent_activity(
        session_name=session_name, limit=limit, since=since
    )


async def get_activity_timeline(limit: int = 20, offset: int = 0) -> dict[str, Any]:
    """Fetch the merged system-wide activity timeline."""
    return await BackboneActivity().get_activity_timeline(limit=limit, offset=offset)
...
class BackboneActivity:
    ...
    async def get_delivery_status(self) -> dict[str, Any]:
        """Fetch aggregated delivery health from the backbone."""
        status, data = await self._request("GET", "/api/deliveries/stats")
        if status == -1:
